...
const ViewOwnerAdmin = () => {
  ...
  useEffect(() => {
    const fetchOwnerData = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/v1/owner/`);
   ...
   const filteredData = ownerData.filter((donar) =>{ 
        const isMatch = Object.values(donar).some((field) =>
     field.toString().toLowerCase().includes(searchTerm.toLowerCase() )
     );

    // Add an additional condition to filter based on "Approved" status
    //const isApproved = donar.status.toLowerCase() === 'approved';
            ...
            <div className="row" style={{marginTop:10}}>
              ...
              <div className="card product-card" style={{marginBottom:10}}>
                <div className="card-body"    >
                      ...
                      <a className="product-title d-block"  >Email: {owner.email} </a>
